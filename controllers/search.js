...
const searchLink = function(request, res, next) {
  ...
  let search = [];
  const getAnime = db.prepare(
    `SELECT * FROM anime WHERE name LIKE '%${name}%' OR name_orig LIKE '%${name}%' LIMIT 50`
  ); /*const getAnime = db.prepare('SELECT * FROM anime');*/
...
const serialsAdd = function(request, res, next) {
  ...
  const CREATE = db.exec(
    "CREATE TABLE IF NOT EXISTS anime(name text, name_orig text, year text, id int, url text)"
  );
  const insert = db.prepare(
    "INSERT INTO anime (name, name_orig, year, id, url) VALUES (@name, @name_orig, @year, @id, @url)"
  );
  let i = 0;
